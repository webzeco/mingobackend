...
exports.getUserOrdersRecord = catchAsync(async (req, res) => {
  const userOrders=await Order.find({guest:false}).populate("orderBy").populate("items.item");
  const requiredOrders=userOrders.filter(order=> order.orderBy.email==req.user.email
  );
  res.status(200).json({
    status: "success",
